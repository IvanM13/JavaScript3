...
const salaryTax = function(x) {
    return x * (1 - 0.13);
}

function checkSalary(n) {
    if (typeof n === 'number' && isFinite(n)) {
      console.log(
        `Размер заработной платы за вычетом налогов равен ${salaryTax(n)} Rub.`);
    } else {
      console.log("Значение задано неверно");
    }
}

let salary = Number(prompt("Введите значение"));
checkSalary(salary);
